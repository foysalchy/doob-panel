  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://doob.dev/api/v1/seller/blog-category?shopId=${shopId}`
      );
    ...
    <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
      ...
      <div className=" my-8">
        <div className="flex flex-wrap gap-4">
          <a
            href="#all"
            className={`px-4 py-2 text-sm font-medium uppercase tracking-wide ${selectedCategory === "all"
              ? "bg-black text-white"
          ...
          {!isLoading &&
            category?.map((category) => (
              <a
                key={category.id}
                href={`#${category.slag}`}
                className={`px-4 py-2 text-sm font-medium uppercase tracking-wide ${selectedCategory === category.slag
                  ? "bg-black text-white"
                  : "bg-gray-300 text-gray-700"
                  }`}
              >
