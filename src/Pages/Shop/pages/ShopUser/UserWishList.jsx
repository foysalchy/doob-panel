...
const UserWishList = () => {
    const { shopUser, shop_id, shopId, setSelectProductData } = useContext(ShopAuthProvider);

    const { data: wishlist = {}, isLoading, refetch } = useQuery({
        queryKey: ["wishlist"],
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/shop/user/wishlist?userId=${shopUser._id}&shopId=${shop_id.shop_id}`);
    ...
    const removeWishlist = (wishListId) => {
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/shop/user/wishlist?wishlistId=${wishListId}`, {
            method: "DELETE",
            headers: {
    ...
    return (
        <div className="max-w-4xl mx-auto ">
            ...
            <p className="text-sm text-gray-600 mt-2">There are {wishlist?.length} products in this list</p>
                    ...
                    <tbody>
                        {wishlist?.data?.map((list) => (
                            <tr className="border-b">
                                ...
                                <td className="py-4 px-6 ">
                                    <button onClick={() => removeWishlist(list?._id)} className="text-gray-400 hover:text-gray-500">
                                        <svg
                                            className="h-6 w-6"
                                            fill="none"
                                            stroke="currentColor"
