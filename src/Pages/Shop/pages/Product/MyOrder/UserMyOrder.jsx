...
const ProductCartLg = ({ order }) => {
  ...
  console.log(shopUser.email, " shopUser.email");
  ...
  return (
    <div className="flex flex-col gap-3">
      {order?.productList?.map((product) => (
        <div
          className="flex border p-4  justify-between gap-4 w-full"
        ...
        >
          ...
          <div className="flex flex-col gap-1 w-full text-center">
            ...
            <p className="text-gray-500 text-center ">{product.quantity}</p>
          </div>
          <div className="flex flex-col gap-1 w-full">
            ...
            {/* {reviewSIngle?.product_id + "   asdfadsf    " + list?.productId} */}
            {order?.status === "delivered" &&
              reviewSIngle?.product_id !== product?.productId && (
                <div>
                  <button
                    className="px-2 py-1 bg-yellow-500 rounded"
                    onClick={() => setOrder_review(product)}
                  >
                    Add Review
                  </button>
                </div>
...
const UserMyOrder = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://doob.dev/api/v1/shop/user/order?shopId=${shop_id.shop_id}&token=${shopUser._id}`
      );
      const data = await res.json();
      return data;
    },
  ...
  const userProductCancel = (orderId, productId, status) => {
    ...
    fetch(
      `https://doob.dev/api/v1/shop/user/order-product-status-update?orderId=${orderId}&productId=${productId}&status=${status}&token=${shopUser._id}`,
      {
        method: "PUT",
  ...
  const handleSubmit = (e) => {
    ...
    console.log(data);
    fetch(`https://doob.dev/api/v1/shop/refund-Order?token=${shopUser._id}`, {
      method: "POST",
      headers: {
  ...
  const cancelNoteSubmit = () => {
    fetch(
      `https://doob.dev/api/v1/shop/user/order-cancel-reason?token=${shopUser._id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
