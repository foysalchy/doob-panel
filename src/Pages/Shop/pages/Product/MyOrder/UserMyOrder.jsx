...
const UserMyOrder = () => {
    ...
    const userProductCancel = (orderId, productId, status) => {
        ...
        fetch(`http://localhost:5000/api/v1/shop/user/order-product-status-update?orderId=${orderId}&productId=${productId}&status=${status}&token=${shopUser._id}`, {
            method: "PUT",
    ...
    const cancelNoteSubmit = () => {
        fetch(`http://localhost:5000/api/v1/shop/user/order-cancel-reason?token=${shopUser._id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
