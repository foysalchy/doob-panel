...
const ProductListCartSm = ({
    ...
    const addToFavorite = (favorite) => {
        ...
        fetch("http://localhost:5000/api/v1/shop/user/wishlist", {
            method: 'POST',
...
const AddToCard = () => {
    ...
    useEffect(() => {
        if (!shopUser) {
            ...
            fetch(`http://localhost:5000/api/v1/shop/user/add-to-cart?userId=${shopUser?._id}&shopId=${shop_id?.shop_id}&token=${shopUser?._id}`)
                .then(res => res.json())
                .then(data => {
