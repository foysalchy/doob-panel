...
const CheckoutModal = ({ modalOpen, setModalOpen, defaultAddress }) => {
    ...
    const submitData = (e) => {
        ...
        fetch('http://localhost:5001/api/v1/shop/upload-Address', {
            method: 'POST',
            headers: {
        ...
        <div>
            <div className={modalOpen ? 'flex' : 'hidden'}>
                <div className="mx-auto py-20">
                    <div className={`fixed z-50 top-0 left-0 flex h-full min-h-screen w-full items-center justify-center bg-black bg-opacity-30 shadow-2xl px-4 py-5 ${setModalOpen ? "block" : "hidden"}`}>
                        <div className="w-full max-w-[800px] h-[90%] rounded-[20px] bg-white pb-10 px-8 text-center md:px-[30px] overflow-scroll">
                            <div className='flex justify-between z-50 pt-4 items-start w-full sticky top-0 bg-white border-b'>
                                <div className='pb-2 text-xl font-bold text-dark text-center sm:text-2xl'>Update your Address</div>
                                <div onClick={() => setModalOpen(!modalOpen)} className='cursor-pointer bg-gray-500 rounded-full px-2.5 mb-2 p-1 text-2xl hover:text-red-500'>
                            ...
                            <form onSubmit={submitData} className='text-start'>
                                <div className="mt-4">
                                    ...
                                    <input type="text" defaultValue={defaultAddress?.fullName} id="fullName" name="fullName" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block py-2 px-3 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border" />

                                    <label htmlFor="mobileNumber" className="mt-4 text-sm font-medium text-gray-700">Mobile Number</label>
                                    <input defaultValue={defaultAddress?.mobileNumber} type="text"
                                    ...
                                    <select onClick={(e) => { setDivision(e.target.value); refetchDistricts(); setDistrict('') }} id="province" name="province" className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        <option selected value={defaultAddress?.province}>{defaultAddress?.province}</option>
                                    ...
                                    <label htmlFor="address" className="mt-4 text-sm font-medium text-gray-700">Address</label>
                                    <input defaultValue={defaultAddress?.address} placeholder="House no. / building / street / area" type="text" id="address" name="address" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block py-2 px-3 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border" />

                                    <label htmlFor="landmark" className="mt-4 text-sm font-medium text-gray-700">Landmark (Optional)</label>
                                    <input defaultValue={defaultAddress?.landmark} type="text" id="landmark" name="landmark" placeholder="E.g. beside train station" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block py-2 px-3 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border" />
                                    ...
                                    <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-20 mt-8 rounded">Save</button>
                                </div>
                            </form>
                        </div>
                    </div>
