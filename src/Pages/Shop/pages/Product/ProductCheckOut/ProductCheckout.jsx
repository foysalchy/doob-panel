...
const ProductCheckout = () => {
    ...
    queryFn: async () => {
      try {
        const res = await fetch(
          `https://salenow-v2-backend.vercel.app/api/v1/shop/user-address?userId=${shopUser?._id}&shopId=${shop_id?.shop_id}`,
          {
            headers: {
  ...
  const checkPromoCode = (e) => {
    ...
    console.log(price);
    fetch(
      `https://salenow-v2-backend.vercel.app/api/v1/shop/user/promocode?shopId=${shopId}&code=${code}&email=${userEmail}&token=${shopUser._id}&price=${price}`,
      {
        headers: {
          "ngrok-skip-browser-warning": "69420",
        },
