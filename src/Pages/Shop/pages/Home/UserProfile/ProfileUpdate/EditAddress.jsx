...
const EditAddress = ({ data, refetch, setEditAddress, setNewAddress }) => {
    ...
    const submitData = (e) => {
        ...
        console.log(data == true ? "https://backend.doob.com.bd/api/v1/shop/upload-Address" : `https://backend.doob.com.bd/api/v1/shop/upload-Address?addressId=${data._id}`);
        const url = data == true ? "https://backend.doob.com.bd/api/v1/shop/upload-Address" : `https://backend.doob.com.bd/api/v1/shop/upload-Address?addressId=${data._id}`
        ...
        <div>
            <form onSubmit={submitData} className='text-start w-full z-50'>
                <div className="mt-4 w-full">
                    ...
                    <input defaultValue={data === true ? shopUser?.phoneNumber : data.mobileNumber} type="text"
                    ...
                    <input defaultValue={data === true ? "" : data.address} placeholder="House no. / building / street / area" type="text" id="address" name="address" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block py-2 px-3 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border" />

                    <label htmlFor="landmark" className="mt-4 text-sm font-medium text-gray-700">Landmark (Optional)</label>
                    <input defaultValue={data === true ? "" : data.landmark} type="text" id="landmark" name="landmark" placeholder="E.g. beside train station" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block py-2 px-3 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border" />
                    ...
                    <div className="mt-4">
                        <label htmlFor="defaultAddress" className="flex items-center">
                            <input type="checkbox" defaultChecked={data === true ? false : data.defaultAddress} id="defaultAddress" name="defaultAddress" className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" />
                        ...
                        <label htmlFor="defaultBillingAddress" className="flex items-center mt-2">
                            <input defaultChecked={data === true ? false : data.defaultBillingAddress} type="checkbox" id="defaultBillingAddress" name="defaultBillingAddress" className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" />
                            <span className="ml-2 text-sm text-gray-700">Default billing address</span>
                        </label>
                    </div>

                    {data === true ? <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-20 mt-8 rounded">Save</button> :
