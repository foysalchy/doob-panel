...
const AddDarazProduct = () => {
    ...
    const { data: Products = [], refetch } = useQuery({
        queryKey: ["allProduct"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/daraz-product/${shopInfo._id}`);
                    ...
                    <div className="mt-4">
                        {
                            loading ?
                                ...
                                <button type='submit'
                                    ...
                                    className={!loading && selectedOption ? "group relative cursor-pointer inline-flex items-center overflow-hidden rounded bg-gray-900 px-8 py-3 text-white focus:outline-none mt-4 " : "group relative inline-flex items-center overflow-hidden rounded bg-gray-700 px-8 py-3 text-white focus:outline-none mt-4 cursor-not-allowed"}
