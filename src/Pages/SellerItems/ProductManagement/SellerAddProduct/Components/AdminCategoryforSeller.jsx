...
const AdminCategoryforSeller = () => {
    ...
    const onHandleExtraCategorys = (selectedOption) => {
        setExtraCategorys([])
        const optionId = selectedOption.value;
        // console.log(`http://localhost:5001/api/v1/admin/category/extraCategory?id=${optionId}`);
        fetch(`http://localhost:5001/api/v1/admin/category/extraCategory?id=${optionId}`)
            .then((res) => res.json())
            .then((data) => {
                console.log(data, '>>>>>');
    ...
    const sortedExtraCategorys = extraCategorys?.filter(warehouse => warehouse.status === 'true').map((itm) => ({
