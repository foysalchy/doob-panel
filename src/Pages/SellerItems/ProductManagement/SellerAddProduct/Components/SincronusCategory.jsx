...
const SincronusCategory = ({ daraz, setDaraz, woo, setWoo, setInputFields, setDarazOption }) => {
    ...
    const { data: megaCategories = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/category/seller/mega-category/get/${shopInfo._id}`);
    ...
    const { data: subCategories = [], refetch: reload } = useQuery({
        ...
        queryFn: async () => {
            if (selectedCategory) {
                const res = await fetch(`http://localhost:5001/api/v1/category/seller/sub-category/get/${shopInfo._id}/${selectedCategory}`);
                const data = await res.json();
                if (data) {
                    if (data?.daraz) {
    ...
    const { data: miniCategories = [], refetch: reMini } = useQuery({
        ...
        queryFn: async () => {
            if (selectedSubcategory) {
                const res = await fetch(`http://localhost:5001/api/v1/category/seller/mini-category/get/${shopInfo._id}/${selectedSubcategory}`);
                const data = await res.json();
                if (data) {
                    if (data?.daraz) {
                        setDarazOption(data?.daraz);
    ...
    const { data: extraCategories = [], refetch: reExtra } = useQuery({
        ...
        queryFn: async () => {
            if (selectedMinicategory) {
                const res = await fetch(`http://localhost:5001/api/v1/category/seller/extra-category/get/${shopInfo._id}/${selectedMinicategory}`);
                const data = await res.json();
                if (data) {
                    if (data?.daraz) {
                        setDarazOption(data?.daraz);
