  ...
  const { data: subscribers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://salenow-v2-backend.vercel.app/api/v1/seller/subscriber-report?shopId=${shopInfo?.shopId}`
      );
                  ...
                  <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                    {currentData?.map((subscriber, index) => (
                      <tr key={subscriber?.id}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                          {handleDate(subscriber.date).formattedDate}{" "}
                          {handleDate(subscriber.date).formattedTime}
                        </td>
