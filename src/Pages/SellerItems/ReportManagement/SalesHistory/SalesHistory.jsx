...
const SalesHistory = () => {
    ...
    const { data: myOrders = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/get-my-perches?shopId=${shopInfo?._id}`);
    ...
    const cancelNoteSubmit = () => {
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/shop/user/order-cancel-reason?token=${shopInfo._id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
                                        ...
                                        <div className="flex items-center justify-between md:w-auto gap-2 md:mt-0 mt-3 w-full">
                                            ...
                                            <div className='flex items-center'>
                                                {order.status === 'pending' && <div >
                                                    {!order.status ? <button onClick={() => setShowAlert(order._id)} className='text-red-500'>  Cancel</button> : <button className=''>{order.status}</button>}
                                                            ...
                                                            <div className="flex gap-x-2 relative">
                                                                ...
                                                                <div>
                                                                    <h2 className="font-medium text-gray-800">
                                                                        {order.product && order.product.name.split(" ").slice(0, 5).join(" ")}
                                                                    </h2>
                                                                    <p className="text-sm font-normal text-gray-600">{order.product.productId}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                    </tbody>
