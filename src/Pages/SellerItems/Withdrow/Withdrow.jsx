...
const Withdraw = () => {
    ...
    const { data: withdrawHistory = [], refetch: reload } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/admin/withdraw-for-shop?shopId=${shopInfo?._id}`);
            const data = await res.json();
            return data.data;
        },
    });
    ...
    useEffect(() => {
        ...
        const total = orders.reduce((acc, order) => {
            return acc + (order.price + parseInt(order.commission) + parseInt(order.handling));
        }, 0);
        setTotalAllTotalPrices(total);
    }, [orders]);

    // Calculate total pending withdrawal
    const totalPendingWithdrawal = withdrawHistory.filter(withdraw => !withdraw.status).reduce((acc, withdraw) => acc + withdraw.amount, 0);

    // Calculate total withdrawal
    const totalWithdrawal = withdrawHistory.filter(withdraw => withdraw.status).reduce((acc, withdraw) => acc + withdraw.amount, 0);

    const total_request = withdrawHistory.length;
    const total_pending = withdrawHistory.filter(withdraw => !withdraw.status).length;
    const total_success = withdrawHistory.filter(withdraw => withdraw.status).length;

    const total_request_amount = withdrawHistory.reduce((acc, withdraw) => acc + withdraw.amount, 0);
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <div className="flex items-center">
                        ...
                        <p className="text-green-500 font-bold">৳{totalPendingWithdrawal.toFixed(2)}</p>
                    </div>
                    <div className="flex items-center">
                        <p className="mr-2">Total Withdrawal</p>
                        <p className="text-green-500 font-bold">৳{totalWithdrawal.toFixed(2)}</p>
                    </div>
                    <div className="flex items-center">
                        <p className="mr-2">Current Balance</p>
                        <p className="text-green-500 font-bold">৳{currentAvailableAmount.toFixed(2)}</p>
                    </div>
                </div>
                <button
            ...
            <table className="w-full border-collapse">
                <thead>
                    <tr className="bg-gray-200">
                        ...
                        <th className="p-3 text-left">Total Amount</th> {/* Add new column for Total Amount */}
                    </tr>
                </thead>
                <tbody>
                    {orders.map((order, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                            ...
                            <td className="p-3">৳{(order.price - (parseInt(order.commission) + parseInt(order.handling))).toFixed(2)}</td> {/* Calculate and display total amount */}
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            {withdraw && (
                <div className="fixed top-0 z-50 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white p-4 max-w-lg rounded-md">
                        ...
                        <p className="mb-4">Are you sure you want to withdraw ৳{currentAvailableAmount}?</p>
                        <input
                            type="number"
                            ...
                            onChange={(e) => setAmount(Math.min(currentAvailableAmount, e.target.value))}
                        />
                        <input
                            type="email"
