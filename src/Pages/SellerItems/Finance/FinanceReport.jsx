...
//             return oneMonthAgo.toISOString().split("T")[0]; // Return the date in YYYY-MM-DD format
//       });
...
const FinanceReport = () => {
      ...
      const fetchFinancialReport = async () => {
            ...
            try {
                  const response = await fetch(apiUrl, {
                        ...
                        body: JSON.stringify({
                              start_date: value.startDate.getTime(),
                              end_date: value.endDate.getTime(),
                              shop_id: shopInfo._id,
                        }),
                  });
      ...
      }, [value]); // Refetch data when date range changes
                        ...
                        <div>
                              <ReactToPrint
                                    trigger={() => (
                                          <button
                                                ref={printButtonRef}
                                                className="bg-black px-5 py-2 text-white rounded hover:bg-black-600"
                                          >
                                                Print
                                          </button>
                                    )}
                        ...
                        >
                              <div className="p-4">
                                    <div className="flex justify-between items-center mb-4">
                                          <div className="text-gray-600">Expected Payout Date:</div>
                                          <div className="text-gray-900">
                                                {new Date(value.startDate).toDateString()} to{" "}
