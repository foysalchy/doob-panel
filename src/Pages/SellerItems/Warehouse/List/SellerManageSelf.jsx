...
const SellerManageSelf = () => {
    ...
    const { data: selfs = [], refetch } = useQuery({
        queryKey: ["selfs"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/warehouse/self/${shopInfo._id}`);
            const data = await res.json();
            return data;
        },
    });

    console.log(`http://localhost:5001/api/v1/seller/warehouse/self/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`http://localhost:5001/api/v1/seller/warehouse/self/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    const DeleteWarehouse = (id) => {
        ...
        Swal.fire({
            ...
            if (result.dismiss === Swal.DismissReason.timer) {
                ...
                fetch(`http://localhost:5001/api/v1/seller/warehouse/self/delete/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
