...
const SellerAddRackModal = ({ setNewData, recall, setOpenModal }) => {
    ...
    const handleWarehouseChange = async (selectedOption) => {
        ...
        const res = await fetch(`http://localhost:5001/api/v1/seller/warehouse/area/${selectedWarehouse}/${shopInfo._id}`);
        const data = await res.json();
        setAreas(data);
    };
    ...
    const UploadArea = (e) => {
        ...
        fetch('http://localhost:5001/api/v1/seller/warehouse/rack', {
            method: 'post',
            headers: {
...
export default SellerAddRackModal;