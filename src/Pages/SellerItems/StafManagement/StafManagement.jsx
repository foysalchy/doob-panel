...
const StafManagement = () => {
    ...
    const { data: staffInfo = [], refetch } = useQuery({
        queryKey: ["staffInfo"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/all-staff?shopEmail=${user.shopEmail}`);
    ...
    if (isDelete) {
        ...
        fetch(`http://localhost:5001/api/v1/seller/staff-delete?email=${deleteId}`, {
            method: "PUT",
            headers: {
