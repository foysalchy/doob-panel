...
const StafManagement = () => {
    ...
    const { data: staffInfo = [], refetch } = useQuery({
        queryKey: ["staffInfo"],
        queryFn: async () => {
            const res = await fetch(`https://doob.dev/api/v1/seller/all-staff?shopEmail=${user.shopId}`);
            const data = await res.json();
            localStorage.setItem('price', JSON.stringify(data?.data));
            return data;
        },
    });
    console.log(`https://doob.dev/api/v1/seller/all-staff?shopEmail=${user.shopId}`);
    ...
    if (isDelete) {
        ...
        fetch(`https://doob.dev/api/v1/seller/staff-delete?email=${deleteId}`, {
            method: "PUT",
            headers: {
                            ...
                            <div className="overflow-hidden border border-gray-200  md:rounded-lg">
                                <table className="min-w-full divide-y divide-gray-200 ">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200  ">
                                        {staffInfo?.users?.map(user => <tr key={user?._id} className='border-b border-black'>
                                            {user?._id === modalOpen?._id &&
                                                <div className='h-0 w-0'>
                                                    <StaffEditModal refetch={refetch} OpenModal={modalOpen} staffInfo={user} setOpenModal={setModalOpen} />
                                                </div>
                                            }
                                            ...
                                            <td className="px-4  border-black py-4 text-sm whitespace-nowrap">
                                                <div className="flex items-center gap-x-6">
                                                    <button onClick={() => DeleteSeller(user?.email)} className="text-gray-500 transition-colors duration-200 dark:hover:text-red-500  hover:text-red-500 focus:outline-none">
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            fill="none"
                                                            viewBox="0 0 24 24"
                                                    ...
                                                    <button onClick={() => setModalOpen(user)} className="text-gray-500 transition-colors duration-200 dark:hover:text-yellow-500  hover:text-yellow-500 focus:outline-none">
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            fill="none"
                                                            viewBox="0 0 24 24"
