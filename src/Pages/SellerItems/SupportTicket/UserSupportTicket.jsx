...
const UserSupportTicketForShop = () => {
    ...
    const { data: tickets = [], refetch, isLoading } = useQuery({
        queryKey: ["userSuport"],
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/user-support?shopId=${shopInfo._id}`);
    ...
    const statusUpdate = ({ id, status }) => {
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/user-support/${id}`, {
            method: 'PUT',
                                        ...
                                        <tr key={ticket?._id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">
                                                {!ticket.status && <span className="relative inline-block px-3 py-1 font-semibold text-yellow-900 leading-tight">
                                                    ...
                                                    || ticket?.status === 'Open' &&
                                                    <button onClick={() => statusUpdate({ id: ticket._id, status: "Closed" })} className="relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight">
                                                        <span
                                                            aria-hidden=""
                                                    ...
                                                    <button onClick={() => statusUpdate({ id: ticket._id, status: "Open" })} className="relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight">
                                                        <span
                                                            aria-hidden=""
                                            ...
                                            <td className="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                                <button onClick={() => fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/user-support/${ticket._id}`, {
                                                    method: 'delete',
                                                    headers: { 'Content-Type': 'application/json' },
