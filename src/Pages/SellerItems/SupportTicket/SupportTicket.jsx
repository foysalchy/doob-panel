...
const SellerSupportTicket = () => {
    ...
    const { data: contact = [], refetch } = useQuery({
        queryKey: ["contact"],
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/support/supportTicketRequest/${user._id}`);
    ...
    return (
        <div>
                    ...
                    <table className=' table-fixed  md:my-10 '>
                        <thead className='text-center  '>
                            <tr className='overflow-x-auto overflow-y-hidden  border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark whitespace-nowrap '>
                                ...
                                <th className="w-1/6 bg-gray-900 border-l border-transparent py-4 px-3 text-lg font-semibold text-white  lg:px-4"> Subject </th>
                                <th className="w-1/6 bg-gray-900 border-l border-transparent py-4 px-3 text-lg font-semibold text-white  lg:px-4">Status</th>

                                <th className="w-1/6 bg-gray-900 border-l  border-transparent py-4 px-3 text-lg font-semibold text-white  lg:px-4">Last Update </th>
                                <th className="w-1/6 bg-gray-900 border-l rounded-tr-md border-transparent py-4 px-3 text-lg font-semibold text-white  lg:px-4"> </th>
                        ...
                        <tbody>
                            {
                                currentData?.map((department) => (
                                    ...
                                    <tr className='overflow-x-auto overflow-y-hidden border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark whitespace-nowrap'>
                                        <td className="border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark">#{department?.ticketId}</td>
                                        <td className="border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark">{truncateSubject(department?.subject)}</td>
                                        <td className="border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark">{!department.status &&
                                            ...
                                            || department.status === 'Open' && <span className="relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight">
                                                <span
                                                    aria-hidden=""
                                            ...
                                            || department.status === 'Closed' && <span className="relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight">
                                                <span
                                                    aria-hidden=""
                                        ...
                                        <td className="border-b border-l border-[#E8E8E8] bg-[#F3F6FF] py-5 px-2 text-center text-base font-medium text-dark">{formatDateTime(department.time)}</td>
            ...
            <AddSupportTicket refetch={refetch} OpenSupport={OpenSupport} setOpenSupport={setOpenSupport} className='w-0 h-0' />
