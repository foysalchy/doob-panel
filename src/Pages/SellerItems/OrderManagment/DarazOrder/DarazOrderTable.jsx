...
const DarazOrderTable = ({ selectedValue, searchValue }) => {
    ...
    console.log(`http://localhost:5001/api/v1/seller/daraz-order?id=${shopInfo._id}`);
    const { data: tData = [], refetch, isLoading } = useQuery({
        queryKey: ["sellerDarazOrder"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/daraz-order?id=${shopInfo._id}&status=${selectedValue}`);
