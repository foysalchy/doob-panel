...
const ScheduleDropOffs = () => {
      ...
      const filteredData = refundData?.filter(
            (itm) =>
                  itm?.orderId.toLowerCase().includes(searchValue.toLowerCase()) ||
                  (itm?.data?.name &&
                        itm?.data?.name.toLowerCase().includes(searchValue.toLowerCase())) ||
                  (itm?.data?.bank_name &&
                        itm?.data?.bank_name
                              .toLowerCase()
                              .includes(searchValue.toLowerCase())) ||
                  (itm?.data?.ac &&
                        itm?.data?.ac.toLowerCase().includes(searchValue.toLowerCase())) ||
                  (itm?.data?.holder &&
                        itm?.data?.holder.toLowerCase().includes(searchValue.toLowerCase())) ||
                  (itm?.data?.paymentMethod &&
                        itm?.data?.paymentMethod
                              .toLowerCase()
                              .includes(searchValue.toLowerCase())) ||
                  (itm?.data?.getway &&
                        itm?.data?.getway.toLowerCase().includes(searchValue.toLowerCase())) ||
      ...
      return (
            <div>
                  <div className="mb-4 mt-4 rounded-md flex items-center  border-gray-400 focus:outline-none bar overflow-hidden  w-full  border">
                        <input
                              placeholder="Search..."
