...
const DarazOrderCheckup = () => {
    ...
    // useEffect(() => {
    //     fetch(`http://localhost:5000/api/v1/seller/daraz-single-order?id=${shopInfo._id}&orderId=${findData?.order_number}`)
    //         .then(res => res.json())
    ...
    const { data: darazSingleOrderProduct = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/api/v1/seller/daraz-single-order?id=${shopInfo._id}&orderId=${findData?.order_number}`);
            const data = await res.json();
            return data.data;
        },
    });
