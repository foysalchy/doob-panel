...
const ExportModal = ({ openModal, setOpenModal, details }) => {
    ...
    const totalPrice = details?.order ? details?.order?.productList?.reduce((total, product) => parseInt(total) + parseInt(product.price), 0) : 0
    console.log(details);
    return (
        <div className={`${openModal ? 'flex' : 'hidden'} fixed z-50 top-0 left-0 h-full min-h-screen w-full items-center justify-center bg-[black] bg-opacity-90 px-20 py-5`}>
            <div className="flex bg-white flex-col max-w-2xl overflow-hidden mt-4 p-6">
                ...
                {details?.refund ? <div className="overflow-x-auto mt-4 transparent-scroll sm:-mx-6 lg:-mx-8">
                                ...
                                <tbody>
                                    <tr className="border-b ">
                                        ...
                                        <td className="whitespace-nowrap w-[60px] word-wrap border-r px-6 py-4 font-medium ">
                                            {details?.refund?.data?.name ? details?.refund?.data?.name : details?.refund?.data?.holder}
                                        </td>
                                        <td className="whitespace-nowrap w-[60px] word-wrap border-r px-6 py-4 font-medium ">
                                            {details?.refund?.data?.account_number ? details?.refund?.data?.account_number : details?.refund?.data?.ac}
                                        </td>
                                        <td className="whitespace-nowrap w-[60px] word-wrap border-r px-6 py-4 font-medium ">
                                            {totalPrice}
                                        </td>
                ...
                </div> : <div className="w-16 h-16 border-4 border-dashed rounded-full animate-spin dark:border-violet-400"></div>}
            </div>
        </div>
    );
};
