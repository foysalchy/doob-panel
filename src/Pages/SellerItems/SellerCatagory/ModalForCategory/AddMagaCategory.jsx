...
const AddMagaCategory = () => {
    ...
    const { data: darazData = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            if (shopInfo.darazLogin) {
                const res = await fetch(`http://localhost:5001/api/v1/daraz/category/${shopInfo._id}`);
    ...
    const { data: wooCategory = [] } = useQuery({
        ...
        queryFn: async () => {
            if (shopInfo.wooLogin) {
                const res = await fetch(`http://localhost:5001/api/v1/woo/category?shopId=${shopInfo._id}`);
                const data = await res.json();
                return data;
            }
    ...
    const postCategory = async (data) => {
        const url = `http://localhost:5001/api/v1/category/seller/add`;
