...
const SubCategoriesManagement = () => {
    ...
    console.log(`https://salenow-v2-backend.vercel.app/api/v1/category/seller/sub/${shopInfo._id}`);

    const { data: categories = [], refetch } = useQuery({
        queryKey: ["categories"],
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/category/seller/sub/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/category/seller/sub/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    const DeleteWarehouse = (id) => {
        ...
        Swal.fire({
            ...
            if (result.dismiss === Swal.DismissReason.timer) {
                ...
                fetch(`https://salenow-v2-backend.vercel.app/api/v1/category/seller/sub/delete/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const handleEdit = async (e, id) => {
        ...
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/category/seller-update-subCategory?id=${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
    ...
    const futuresUpdate = (id, status) => {
        ...
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/category/seller-update-subCategory-feature?id=${id}&status=${status}`, {
            method: "PUT",
                                ...
                                currentData.map((warehouse, index) => (
                                    ...
                                    <tr key={index + warehouse?._id + 1} className='border-b'>
                                        <td className="px-4 py-3">
                                            <img src={warehouse?.img && warehouse?.img} alt="" className="w-[50px] rounded-lg" />
                                        </td>
                                        ...
                                        <td className="px-4  text-2xl flex gap-2 py-6 items-center text-gray-100">
                                            ...
                                            <BiEdit className="text-yellow-500 cursor-pointer"
                                                onClick={() => handleViewDetails(warehouse?._id, warehouse)}

                                            />

                                        </td>
                                        <td>
                                            {/* <button onClick={() => futuresUpdate(warehouse?._id, warehouse?.feature ? false : true)} className={` ${warehouse?.feature ? "bg-green-500 hover:bg-green-400" : "bg-red-500 hover:bg-red-400"}inline-flex items-center justify-center py-1 px-4  rounded shadow-md  focus:shadow-outline focus:outline-none`}>Feature</button> */}
