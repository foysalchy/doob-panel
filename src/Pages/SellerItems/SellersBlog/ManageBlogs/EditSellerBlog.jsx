...
const EditSellerBlog = ({ OpenModal, setOpenModal, BlogInfo, refetch }) => {
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            const response = await fetch(`https://backend.doob.com.bd/api/v1/image/upload-image?shopId=${shopInfo._id}`, {
                method: "POST",
                body: formData,
            });
    ...
    const handleBlogUpdate = async (e) => {
        ...
        try {
            const response = await fetch(`https://backend.doob.com.bd/api/v1/seller/blog/update-blog/${BlogInfo._id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className={`fixed z-50 top-0 left-0 flex h-full min-h-screen w-full items-center justify-center bg-black bg-opacity-90 px-4 py-5 ${OpenModal ? "block" : "hidden"}`}>
            <div className="w-full max-w-[800px]  rounded-[20px] bg-white pb-10 px-8 text-center md:px-[30px] ">
                ...
                <form className='h-[500px] overflow-y-scroll' onSubmit={handleBlogUpdate}>
                    ...
                    <div className='mb-4'>
                        <label htmlFor="dropzone-file" className="flex flex-col items-center w-full p-5 mx-auto mt-2 text-center bg-white border-2 border-gray-300 border-dashed cursor-pointer rounded-xl">
                            <img srcSet={previewUrl || BlogInfo.img} src={previewUrl || BlogInfo.img} alt="File Preview" className="mt-2 w-8 h-8" />
                    ...
                    <input name='MetaTag' className='w-full p-2 my-4 border' defaultValue={BlogInfo.MetaTag} />
                    <textarea className='w-full border p-2' defaultValue={BlogInfo.MetaDescription} name="MetaDescription" id="" cols="20" rows="2"></textarea>
                    ...
                    <div className='flex justify-start'>
                        <button type='submit' className="group relative inline-flex items-center overflow-hidden rounded bg-gray-900 px-8 py-3 text-white focus:outline-none focus:ring active:bg-gray-500 mt-4">
                            ...
                            <span className="text-sm font-medium transition-all group-hover:ms-4">Update Blog</span>
                        </button>
                    </div>
                </form>
            </div>
