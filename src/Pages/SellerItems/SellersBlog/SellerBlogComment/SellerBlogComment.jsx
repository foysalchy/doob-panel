...
const SellerBlogComment = () => {
    ...
    const { data: adminBlogsComments = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/blog-comments?shopId=${shopInfo._id}`);
                    ...
                    {blogsComments.map((blog) => (
                        <tr>
                            ...
                            <td className="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                {blog.status ? <button onClick={() => publishBlogComment(blog._id,)} className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 bg-gray-800">
                                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                                    <h2 className="text-sm font-normal text-emerald-500">
                                        Active
                                    </h2>
                                </button> :
                                    <button onClick={() => publishBlogComment(blog._id)} className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 bg-gray-800">
...
export default SellerBlogComment;