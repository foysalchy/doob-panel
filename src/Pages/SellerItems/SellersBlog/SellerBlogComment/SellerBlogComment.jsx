...
const SellerBlogComment = () => {
  ...
  const { data: adminBlogsComments = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://salenow-v2-backend.vercel.app/api/v1/seller/blog-comments?shopId=${shopInfo._id}`
      );
  ...
  const publishBlogComment = (id, timestamp, status) => {
    fetch(
      `https://salenow-v2-backend.vercel.app/api/v1/admin/blog-comments-update-status?blogId=${id}&commentId=${timestamp}&status=${status}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...
          {blogsComments.map((blog) => (
            <tr>
              ...
              <td className="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                {blog.status ? (
                  <button
                    onClick={() => publishBlogComment(blog._id)}
