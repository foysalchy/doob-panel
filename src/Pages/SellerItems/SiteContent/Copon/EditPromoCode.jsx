...
const EditPromoCode = ({ data, setOpenModal, refetch }) => { // Component name changed to EditPromo
    ...
    return (
        <div className={`fixed left-0 top-0 right-0 bottom-0 flex h-full min-h-screen w-full z-[1000] bg-[#0000005b] items-center justify-center bg-dark/90 px-4 py-5 ${open ? "block" : "hidden"
            }`}>
            ...
            <form
                ...
                className="bg-white h-[80%] w-[60%] overflow-hidden shadow-md rounded px-8 pt-6 pb-8 mb-4"
            >
                <h2 className="text-2xl  font-semibold text-black mb-6 ">
                    Edit Promo
                    <div onClick={() => setOpenModal(false)} className="bg-gray-900 cursor-pointer flex items-center justify-center text-white w-[30px] h-[30px] float-right rounded-full text-xl"> <RxCross2 /></div>
                </h2>
                <div className="h-[80%]  p-3 overflow-y-auto overflow-x-hidden">
                    <div className="mb-4">
                        <label
                            htmlFor="code"
                    ...
                    <div className="mb-4">
                        ...
                        <Select
                            ...
                            options={options}
                            value={formData.selectedGmails.map((email) => ({ value: email, label: email }))} // Map emails to options format
