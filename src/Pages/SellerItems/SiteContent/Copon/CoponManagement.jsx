...
const CoponManagement = () => {
    ...
    const { data: faqs = [], refetch } = useQuery({
        queryKey: ["faqs"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/promo-code/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`http://localhost:5001/api/v1/seller/promo-code/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    if (isDelete) {
        ...
        fetch(`http://localhost:5001/api/v1/seller/promo-code/delete/${deleteId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
            },
