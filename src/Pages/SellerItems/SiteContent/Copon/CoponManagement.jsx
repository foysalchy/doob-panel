...
const CoponManagement = () => {
    ...
    const { data: faqs = [], refetch } = useQuery({
        queryKey: ["faqs"],
        queryFn: async () => {
            const res = await fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/promo-code/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/promo-code/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    if (isDelete) {
        ...
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/seller/promo-code/delete/${deleteId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
            },
                                                    ...
                                                    <div className="inline-flex items-center gap-3">
                                                        <div className="inline-flex items-center gap-x-3">
                                                            ...
                                                            <div className="font-medium text-gray-800  whitespace-pre-wrap ">
                                                                ...
                                                                <button
                                                                    className={` ml-4 ${copiedIndex === index ? ' cursor-not-allowed' : ''}`}
                                                                    onClick={() => handleCopyClick(faq?.code, index)}
                                                                    disabled={copiedIndex === index}
                                                                >
                                                                    {copiedIndex === index ? <FaRegCopy className='text-green-700' /> : <FaRegCopy className='text-gray-900' />}
                                                                </button>
                                                            </div>
