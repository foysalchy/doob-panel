...
const FeatureManagement = () => {
    ...
    const { data: faqs = [], refetch } = useQuery({
        queryKey: ["faqs"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/seller/feature/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`http://localhost:5001/api/v1/seller/feature/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    if (isDelete) {
        ...
        fetch(`http://localhost:5001/api/v1/seller/feature/delete/${deleteId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
            },
