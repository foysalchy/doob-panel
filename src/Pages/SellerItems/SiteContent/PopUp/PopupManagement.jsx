...
const PopupManagement = () => {
    ...
    const { data: faqs, refetch } = useQuery({
        queryKey: "faqs",
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/api/v1/seller/popup/${shopInfo._id}`);
    ...
    const updateStatus = (id, status) => {
        fetch(`http://localhost:5000/api/v1/seller/popup/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div>
            ...
            <section className=" mt-4 mx-auto">
                ...
                {faqs ? <div className="max-w-2xl overflow-hidden bg-white rounded-lg shadow-md dark:bg-gray-800">
                    <img className="object-cover w-full h-64" srcSet={faqs?.image} alt="Article" />
                    <div className="p-6">
                        <div>
                            <span className="text-xs font-medium text-blue-600 uppercase dark:text-blue-400">Ads Link</span>
                            <a href="#" className="block mt-2 text-xl font-semibold text-gray-800 transition-colors duration-300 transform dark:text-white hover:text-gray-600 hover:underline" tabIndex={0} role="link">{faqs?.link}</a>
                ...
                </div> : <h1> sorry you have no ads right now</h1>}
            </section>
        </div>
    );
};
