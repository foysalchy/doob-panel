...
const MiniCategoryManagement = () => {
  ...
  const { data: miniCategory = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://salenow-v2-backend.vercel.app/api/v1/admin/category/miniCategories"
      );
  ...
  const statusUpdate = (id, status) => {
    fetch(
      `https://salenow-v2-backend.vercel.app/api/v1/admin/category/miniCategory?id=${id}&status=${status}`,
      {
        method: "PUT",
        headers: {
                        ...
                        >
                          ...
                          <form onSubmit={(e) => handleEdit(e, item?._id)}>
                            ...
                            <div className="flex flex-col items-start gap-1">
                              ...
                              <input
                                defaultValue={item?.miniCategoryName}
                                type="text"
                                name="name"
                                className="border border-gray-500 p-1 rounded mb-3 w-full"
                              />
