...
import AdminEditStaffModal from './AdminEditStaffModal';
import Swal from 'sweetalert2';
import DeleteModal from '../../../Common/DeleteModal';

const AdminStaffManagement = () => {
    //http://localhost:5001/api/v1/admin/staff-role
    const [modalOpen, setModalOpen] = useState(false);
    const { data: staffInfo = [], refetch } = useQuery({
        queryKey: ["staffInfo"],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5001/api/v1/admin/staff-role`);
    ...
    if (isDelete) {
        ...
        fetch(`http://localhost:5001/api/v1/admin/staff-delete?email=${deleteId}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div>
            ...
            <Link
                ...
                to="/admin/staff-management/new-staff"
            >
                            ...
                            <div className="overflow-hidden border border-gray-200  md:rounded-lg">
                                <table className="min-w-full divide-y divide-gray-200 ">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200  ">
                                        {staffInfo?.users?.map(user => <tr key={user?._id}>

                                            {user?._id === modalOpen?._id &&
                                                <div className='h-0 w-0'>
                                                    <AdminEditStaffModal refetch={refetch} OpenModal={modalOpen} staffInfo={user} setOpenModal={setModalOpen} />
                                                </div>
                                            }
