...
const AdminPosProductsDetails = ({ invoice, open, setOpen }) => {
    ...
    const fetchData = () => {
        fetch(`http://localhost:5001/api/v1/seller/seller-user?shopId=${shopInfo.shopId}&${searchType}=${searchValue}`).then((res) => res.json()).then((data) => {
            if (data.userInfo) {
        ...
        <div>
            {
                open && <div className='bg-[#050505bc]  w-screen  fixed top-0 bottom-0 left-0 right-0 flex items-center justify-center z-[1000]'>
                    <div className="w-[600px] max-h-[90%] overflow-y-auto p-6 bg-white rounded-lg">
                        ...
                        <AdminPosInvoice invoiceData={invoice} setInvoiceOpen={setInvoiceOpen} invoiceOpen={invoiceOpen} />
                    </div>
                </div>
            }
        </div>
