...
const AdminFeatureImage = () => {
    ...
    const EditStatus = (id, status) => {
        ...
        setLoading(true)
        fetch(`https://backend.doob.com.bd/api/v1/admin/feature-image?id=${id}&status=${status}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
            ...
            <section className=" mt-4 mx-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                        {featureImage?.sort((a, b) => parseInt(a.index) - parseInt(b.index))?.map(itm => <tr key={itm?._id}>
                            ...
                            <div className={`fixed z-[100] flex items-center justify-center ${openModal?._id === itm?._id ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100`}>
                                <div className={`absolute md:w-[500px] w-full rounded-sm bg-white p-3 pb-5 text-center drop-shadow-2xl ${openModal?._id === itm?._id ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'} `}>
                                    ...
                                    <form onSubmit={(e) => handleEdit(e, itm?._id)}>
                                        ...
                                        <img src={itm?.image} alt="" className="w-[100px] h-[100px] rounded" />
