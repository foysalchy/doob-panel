...
const AdminAnouncement = () => {
    ...
    const EditStatus = (id, status) => {
        ...
        setLoading(true)
        fetch(`http://localhost:5001/api/v1/admin/announcement?AnointmentId=${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
    ...
    const onDelete = (id) => {
        ...
        fetch(`http://localhost:5001/api/v1/admin/announcement?AnointmentId=${id}`, {
            ...
            console.log(data, `http://localhost:5001/api/v1/admin/announcement?AnointmentId=${id}`);
            setLoading(false)
            Swal.fire(`Seller disable ${status} `, '', 'success');
            refetch()
        })
            ...
            <section className=" mt-4 mx-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                        {popupData.map(itm => <tr key={itm?._id}>
