...
const ViewTicket = ({ viewTicket, setViewTicket, ticketDetails, refetch }) => {
      ...
      const commentSubmit = async (event) => {
            ...
            fetch(`https://doob.dev/api/v1/support/support-ticket/${id}`, {
                  method: "PUT",
                  headers: {
      ...
      const handleHoverLeaveComment = (commentId) => {
            updateImageState(commentId, {
                  ...imageStates[commentId],
                  isHovered: false,
            });
      };
      ...
      const handleClickComment = (commentId) => {
            updateImageState(commentId, {
                  ...imageStates[commentId],
                  isFullscreen: !imageStates[commentId]?.isFullscreen,
            });
      };
                                    ...
                                    <div className="w-full max-w-[800px] h-[90%]  rounded-[20px]  bg-white relative pb-10 px-8 text-center md:px-[30px] ">
                                          <div className="flex justify-between z-50 pt-4 items-start w-full sticky top-0 bg-white border-b">
                                                <div className="pb-2 text-xl font-bold text-dark text-center sm:text-2xl">
                                                      {ticketDetails.userInfo.name}'s Message
                                                </div>
                                                <div
                                          ...
                                          <div className="bar overflow-y-auto h-[90%]">
                                                ...
                                                <div>
                                                      {ticketDetails?.comments?.map((comment) => (
                                                            <div
                                                                  key={comment?._id}
                                                                  className={`grid grid-cols-4 gap-2 mb-8 border rounded-lg  shadow-lg ${comment?.user !== ticketDetails?.userInfo?.name &&
                                                                        "bg-green-300 "
                                                                        } `}
                                                            >
