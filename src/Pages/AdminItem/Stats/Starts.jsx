...
const Starts = () => {
    ...
    const { data: newUsers = [], refetch } = useQuery({
        queryKey: ["newUser"],
        queryFn: async () => {
            const res = await fetch("https://salenow-v2-backend.vercel.app/api/v1/admin/previous-week-users");
                ...
                <div className=" pt-6 2xl:container">
                    <div>
                        <div className="flex flex-wrap">
                                    ...
                                    <div className="flex-auto p-4">
                                        <div className="flex ">
                                            <div className="relative w-full pr-4 max-w-full flex-grow flex-1">
                                                ...
                                                <span className="font-bold text-xl">{orderData.length}</span>
                                            </div>
                                    ...
                                    <div className="flex-auto px-4 py-2">
                                        <div className="flex ">
                                            <div className="relative w-full pr-4 max-w-full flex-grow flex-1">
                                                ...
                                                <button onClick={() => setShowSeller(true)} className="text-blue-500">view</button>
                                            </div>
                                            <div className="relative w-auto pl-4 flex-initial">
                            ...
                            {/* seller modal */}
                            <div>
                                <div onClick={() => setShowSeller(false)} className={`fixed z-[100] flex items-center justify-center ${showSeller ? 'visible opacity-100' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100 dark:bg-white/10`}>
                                    <div onClick={(e_) => e_.stopPropagation()} className={`text- absolute w-[600px] rounded-sm bg-white p-6 drop-shadow-lg dark:bg-white dark:text-black ${showSeller ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'}`}>
                                        <main>
                                            <h1 className="font-semibold border-b pb-3">Top 10 Seller</h1>
                                            <ul>
                                                ...
                                                {
                                                    sellerData && sellerData?.slice(0, 10).map(itm => (
                                                        <li className='grid grid-cols-3 gap-4 py-2 border-b border-gray-400 text-sm text-gray-500' key={itm?._id}>
                                                            <div className='text-nowrap w-full text-xs text-start'>{itm?.name}</div>
                                                            <div className='text-nowrap w-full text-xs text-start'>{itm?.shopName}</div>
                                        ...
                                        <div className="flex justify-between">
                                            <button onClick={() => setShowSeller(false)} className="rounded-sm border border-red-600 px-6 py-[6px] text-red-600 duration-150 hover:bg-red-600 hover:text-white">Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="flex-auto p-4">
                                        <div className="flex ">
                                            <div className="relative w-full pr-4 max-w-full flex-grow flex-1">
                                                ...
                                                <span className="font-bold text-xl">{newUsers?.newUsersCount ? newUsers?.newUsersCount : 0}</span>
                                            </div>
                                            <div className="relative w-auto pl-4 flex-initial">
                                                <div className="text-white p-3 text-center inline-flex items-center justify-center w-12 h-12 shadow-lg rounded-full bg-orange-500">
                                                    <BiSolidUser />
            ...
            <div className="grid grid-cols-3 gap-6">
                ...
                <div className="bg-[#1caf12] p-4 h-[140px] rounded-md flex items-center justify-between">
                    <div>
                        ...
                        <h1 className="text-white font-bold text-4xl ">{orderData.length}</h1>
                    </div>
                    <div>
                ...
                <div className="bg-[#1b4ffa] p-4 h-[140px] rounded-md flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-white mt-1">
                            <small className={!newUsers?.percentageChange > 0 ? "text-red-500 mr-2" : "text-green-500 mr-2"}>
                                <i className="fas fa-arrow-down" /> {newUsers?.percentageChange}%
                            </small>
                            <span className="whitespace-nowrap">Since last week</span>
                        </p>
                        <h1 className="text-white font-bold text-4xl ">{newUsers?.newUsersCount ? newUsers?.newUsersCount : 0}</h1>
                    </div>
                    <div>
                        ...
                        <div className="block w-full overflow-x-auto">
                            <table className="items-center bg-transparent w-full border-collapse ">
                                ...
                                <tbody>
                                    ...
                                    {
                                        sellerData && sellerData?.slice(0, 10).map((item, index) => (
                                            <tr>
                                                <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700">
                                                    {index + 1} {/* Assuming there's a name property in your data */}
                                                </th>
                                                <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700">
                                                    {item?.name ? item?.name : 'empty'} {/* Assuming there's a name property in your data */}
                                                </th>
...

                                    {
                                        sellerData && sellerData?.slice(0, 20).map((item, index) => (
                                            <tr>
                                                ...
                                                <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700">
                                                    <img src={`${item?.featuredImage?.src}`} className="w-[60px] h-[60px] rounded-md" />
                                                </th>
