  ...
  const { data: options = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://salenow-v2-backend.vercel.app/api/v1/users"
      );
            ...
            <div className="mt-4">
              ...
              <ul className="flex items-start flex-wrap gap-2 mt-2">
                {selectedOptions?.map((selected) => (
                  <li
                    key={selected}
                    className="flex items-center bg-gray-200 px-2 py-0.5 rounded mb-2"
                  >
                    <span className="mr-2 text-sm text-gray-800">
                      {
                        options?.find((option) => option?.email == selected)
                          ?.email
                      }
                    </span>
                    <button
        ...
        {selectedMedia !== "Select Your Option" && (
          <div>
            <input
              ...
              className="flex-grow mt-4 w-full re h-12 px-4 mb-2 transition duration-200 bg-white border border-gray-300 rounded shadow-sm appearance-none focus:border-deep-purple-400 focus:outline-none focus:shadow-outline"
            />
