...
const SellerDomainManagement = () => {
    ...
    const { data: shops = [], refetch, isLoading } = useQuery({
        queryKey: ["shops"],
        queryFn: async () => {
            const res = await fetch("http://localhost:5001/api/v1/shop");
    ...
    const UpdateStatus = (id, status) => {
        ...
        fetch(`http://localhost:5001/api/v1/shop/domainstatus/${id}?status=${status}`,
            {
                method: "Put",
                headers: {
    ...
    if (isDelete) {
        ...
        fetch(`http://localhost:5001/api/v1/shop/delete/${deleteId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const updateShopStatus = (id, status) => {
        ...
        fetch(`http://localhost:5001/api/v1/seller/update-shopInfo-for-status?id=${id}&status=${status}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
