...
const Daraz = () => {
    ...
    const { data: keys = [], refetch } = useQuery({
        queryKey: ["key"],
        queryFn: async () => {
            const res = await fetch("http://localhost:5000/api/v1/daraz/get-key");
    ...
    const uploadData = (e) => {
        ...
        console.log(data);
        fetch('http://localhost:5000/api/v1/daraz/add-key', {
            method: "PUT",
            headers: {
    ...
    return (
        <div>
            ...
            <form onSubmit={uploadData} className=' mr-4 border border-dashed border-black p-6'>
                <div>
                    <label for="App Key" className="block text-sm mb-2 text-gray-900 ">App Key</label>
                ...
                <div>
                    <label for="App Secret Key" className="block text-sm mb-2 text-gray-900 ">App Secret Key</label>
