...
const EditSellerInfo = ({ OpenModal, setOpenModal, SellerInfo, refetch }) => {
    ...
    const handleBlogUpdate = async (e) => {
        ...
        try {
            fetch(`http://localhost:5001/api/v1/admin/seller/update-sellerinfo/${SellerInfo._id}`, {
                method: 'PUT',
                headers: {
        ...
        <div className={`fixed z-50 top-0 left-0 flex h-full min-h-screen w-full items-center justify-center bg-black bg-opacity-90 px-4 py-5 ${OpenModal ? "block" : "hidden"}`}>
            <div className="w-full max-w-[800px]  rounded-[20px] bg-white pb-10 px-8 text-center md:px-[30px]">
                <div className='flex justify-between z-50 pt-4 items-start w-full sticky top-0 bg-white border-b'>
                    <div className='pb-2 text-xl font-bold text-dark text-center sm:text-2xl'>Edit Seller Info</div>
                ...
                <form className=' overflow-y-scroll' onSubmit={handleBlogUpdate}>
                    ...
                    <input name='name' className='w-full p-2 my-4 border border-black' defaultValue={SellerInfo.name} />
                    <input name='email' className='w-full p-2 my-4 border border-black' defaultValue={SellerInfo.email} />
                    <input name='userId' className='w-full p-2 my-4 border border-black' defaultValue={SellerInfo.userId} />
                    ...
                    <div className='flex justify-start'>
                        <button type='submit' className="group relative inline-flex items-center overflow-hidden rounded bg-gray-900 px-8 py-3 text-white focus:outline-none focus:ring active:bg-gray-500 mt-4">
                            ...
                            <span className="text-sm font-medium transition-all group-hover:ms-4">Update Seller Info</span>
                        </button>
                    </div>
                </form>
            </div>
