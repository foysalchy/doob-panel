...
const WarehouseModal = ({ modalOpen, setModalOpen, product, doobProduct }) => {
    ...
    const { data: warehouses = [], refetch, isRefetching } = useQuery({
        queryKey: ["warehouses"],
        queryFn: async () => {
            const getWarehouseApiUrl = "https://salenow-v2-backend.vercel.app/api/v1/admin/warehouse";
    ...
    const updateInfo = (e) => {
        ...
        fetch(`https://salenow-v2-backend.vercel.app/api/v1/admin/update-product-info?productId=${product._id}`, {
            method: 'PUT',
            headers: {
                'Content-Type':
                    'application/json'
