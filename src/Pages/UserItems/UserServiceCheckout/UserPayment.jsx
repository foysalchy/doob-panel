...
const UserPayment = () => {
    ...
    const orderSubmit = () => {
        ...
        setPassData(data);
        fetch(`http://localhost:5000/api/v1/site-user/order`, {
            method: "POST",
                ...
                <div className="grid md:grid-cols-4 grid-cols-1 md:col-span-3 gap-4">
                    ...
                    <a href="#scrollDestination">
                        <div onClick={() => setPayment({ Getaway: "CashOnDelivery" })} className={`${payment?.Getaway === 'CashOnDelivery' && 'shadow-lg shadow-gray-700'}  border border-gray-600 flex md:flex-col flex-row items-center justify-center  gap-2 rounded p-4 md:w-[200px] md:h-[220px] w-full h-[50px] overflow-hidden`}>
                            <img
                                alt="Developer"
                                src="http://localhost:5000/api/v1/image/658ec416b689ffabf15d9fb6.jpg"
                                srcSet="http://localhost:5000/api/v1/image/658ec416b689ffabf15d9fb6.jpg"
                                className="md:h-[120px] md:w-[120px] w-[30px] h-[40px] object-cover"
                            />
                    ...
                    {message.map((mess, i) => <div className='py-2 bg-yellow-200 px-10' key={i}>{mess}</div>)}
                    {
                        payment.Getaway === "Bank" && <div className='flex flex-col gap-2 text-xs'>
                            ...
                            {<label htmlFor="fileInput" className="relative cursor-pointer  px-10 py-2 rounded-md text-black border">
                                ...
                                <span className="ml-2">{fileName ? fileName : "Choose Your Payment Documents"}</span>
                                <input id="fileInput" className="hidden" type="file" onChange={handleFileChange} />
                                {previewUrl && <img src={previewUrl} alt="File" className="mt-2 max-h-20 object-cover" />}
                            </label>}
                        </div>
