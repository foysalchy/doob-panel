...
const EmailVerify = () => {
    ...
    useEffect(() => {
        const verifyEmail = async () => {
            try {
                const response = await fetch('https://backend.doob.com.bd/api/v1/auth/verify-email', {
                    method: 'PUT',
                    headers: {
                            ...
                            <div className="max-w-xl mb-10 md:mx-auto sm:text-center lg:max-w-2xl md:mb-12">
                                <h2 className="max-w-lg mb-6 font-sans text-3xl font-bold leading-none tracking-tight text-white sm:text-4xl md:mx-auto">
                                    <span className="relative inline-block">
                                        ...
                                        <span className="relative">{loading ? 'Loading' : "Your"}</span>
                                    </span>{' '}
                                    {success ? "Email verification successful!" : "mail verification failed. Please try again later."}
                                </h2>
