...
const ShopPath = [
    {
        ...
        loader: async ({ params }) => {
            const id = params.id;
            return fetch(`http://localhost:5001/api/v1/shop/${id}`);
        },
    },
    {
        path: ':id/sign-in',
...

    {
        ...
        loader: async ({ params }) => {
            ...
            return fetch(`http://localhost:5001/api/v1/shop/product/${id}/product/${productID}`);
        },
    },
    {
        ...
        loader: async ({ params }) => {
            ...
            if (categoryName !== null) {
                ...
                const response = await fetch(`http://localhost:5001/api/v1/shop/product/${shopId}/categories?category=${encodeURIComponent(categoryName)}`);
    ...
    },
    {
        ...
        loader: async ({ params }) => {
            const id = params.id;
            return fetch(`http://localhost:5001/api/v1/seller/blog/${id}`);
        },
    },
    {
        path: ':id/blog/:blogId',
        ...
        loader: async ({ params }) => {
            ...
            const response = await fetch(`http://localhost:5001/api/v1/seller/blog/${id}/${blogId}`);
            const data = await response.json();
            return data;
        },
    },
    {
        ...
        loader: async ({ params }) => {
            ...
            const response = await fetch(`http://localhost:5001/api/v1/seller/page/${shopId}/${pageId}`);
            const data = await response.json();
            return data;
        },
    },
    ...
    },
    {
        ...
        loader: async (params) => {
            ...
            if (shopIdMatch) {
                ...
                try {
                    const response = await fetch(`http://localhost:5001/api/v1/seller/payment-getaway/${shopId}`);
                    const data = await response.json();
    ...
    },
    {
        ...
        loader: async (params) => {
            ...
            if (shopIdMatch && userIdMatch) {
                ...
                try {
                    const response = await fetch(`http://localhost:5001/api/v1/shop/user-address?userId=${userId}&shopId=${shopId}&token=${userId}`, {
                        headers: {
