...
const Search = () => {
    ...
    return (
        <div>
            <div>
                <div className={`fixed z-50 left-0 bg-opacity-70 bg-gray-500 top-0 flex h-full min-h-screen w-full items-center justify-center bg-dark/90 px-4 py-5 ${search ? "block" : "hidden"
                    }`}>
                    <div>
                            ...
                            <div className="w-96 " >
                                <div className='relative '>
                                    ...
                                    <span className="absolute inset-y-0 end-0 grid w-10 place-content-center">
                                        <button type="button" className="text-gray-600 hover:text-gray-700" onClick={searchData}>
                        ...
                        <div className="mt-4 w-[800px]">
                            <div className='flex flex-wrap justify-center gap-2'>
                                {searchHistory.slice(0, 10).map((item, index) => (
                                    <button className='border-2 text-sm px-2 rounded-2xl ' onClick={() => setSearchTerm(item.term)} key={index}>{item.term}</button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mt-4 ">
                            ...
                            <ul>
                                {searchResults?.data?.productCollections?.filter((p) => p.adminWare).map((product, index) => (
                                    <li>
                                        <Link onClick={() => { setSearch(false), setSearchHistory() }} to={`/products/${product._id}`} className='text-white' key={index}>{product?.name}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div className="mt-4">
                            ...
                            <ul>
                                {searchResults?.data?.serviceCollections?.map((service, index) => (
                                    <li key={index}>  <Link to={`/service/${service._id}`} onClick={() => { setSearch(false), setSearchHistory() }}>
                                        {service.title}
                                    </Link></li>
                                ))}
                            </ul>
